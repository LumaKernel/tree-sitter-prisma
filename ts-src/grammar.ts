...
module.exports = grammar({
  ...
  rules: {
    ...
    model_block: ($) =>
      seq("model", $.identifier, "{", $._model_block_content, "}"),
    _model_block_content: ($) =>
      repeat1(choice($.model_field, $.model_multi_attribute)),
    model_field: ($) =>
      seq(
        field("field_name", $.identifier),
        $.field_type,
        repeat($.model_single_attribute)
      ),

    enum_block: ($) =>
      seq("enum", $.identifier, "{", $._enum_block_content, "}"),
    _enum_block_content: ($) => repeat1($.identifier),

    model_single_attribute: ($) =>
      seq("@", $.attribute_specifier, optional($._model_attribute_args)),
    model_multi_attribute: ($) =>
      seq("@@", $.attribute_specifier, optional($._model_attribute_args)),
    _model_attribute_args: ($) =>
      seq("(", optional($._model_attribute_arg_separated), ")"),

    attribute_specifier: ($) =>
      seq($.identifier, repeat(seq(".", $.identifier))),

    _model_attribute_arg_separated: ($) =>
      seq($._model_attribute_arg, repeat(seq(",", $._model_attribute_arg))),
    _model_attribute_arg: ($) =>
      choice($._model_attribute_arg_labeled, $._model_attribute_arg_no_label),
    _model_attribute_arg_labeled: ($) =>
      seq(field("label", $.identifier), ":", $._model_attribute_arg_no_label),
    _model_attribute_arg_no_label: ($) =>
      choice($._expression, $._field_array, $.identifier),
    ...
    _field_array: ($) =>
      seq("[", optional(field("fields", $._identifier_separated)), "]"),
    ...
    apply_function: ($) =>
      seq(
        ...
        "(",
        optional($._expression_separated),
        ")"
      ),
